...
These images are used in the behavioral experiment.

Use these command to convert training images into a video
    avconv -i "s7_train_%d_0.png" -s:v 400x400 s7.ogg
...
def render_stimulus(name, shape):
    r, theta, _ = shape.viewpoint[0]
    # during training, subjects see views: theta = 270, 285, 300, 345, 0, 15
    ts = range(270, 300) + range(345, 375)
    ts = list(reversed(ts)) + ts
    for i, t in enumerate(ts):
        shape.viewpoint = [(r, (theta + t) % 360, 45)]
        fwm.save_render("stimuli/exp2/behavioral/train/{0:s}_train_{1:d}.png".format(name, i), shape)
    ...
    fwm.save_render("stimuli/exp2/behavioral/{0:s}_inter.png".format(name), shape)
    ...
    fwm.save_render("stimuli/exp2/behavioral/{0:s}_extra.png".format(name), shape)
